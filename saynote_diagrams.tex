% SayNote Diagrams
% This file contains TikZ diagrams for the SayNote documentation
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
        block/.style={rectangle, draw, rounded corners, minimum width=2.5cm, minimum height=1cm, align=center, fill=SayNoteLightGray},
        cloud/.style={draw, ellipse, fill=SayNoteLightGray, minimum width=2.5cm, minimum height=1.5cm, align=center},
        arrow/.style={->, >=stealth, thick, color=SayNoteBlue},
        every node/.style={font=\small}
    ]
    
    % Client Layer
    \node[block, fill=SayNoteBlue!20] (mobile) at (0,0) {Mobile Application\\(Expo/React Native)};
    
    % API Layer
    \node[block, fill=SayNoteBlue!30] (api) at (0,-2.5) {RESTful API\\(Node.js)};
    
    % Services
    \node[block, fill=SayNoteBlue!40] (auth) at (-4,-5) {Authentication\\Service};
    \node[block, fill=SayNoteBlue!40] (notes) at (0,-5) {Notes\\Service};
    \node[block, fill=SayNoteBlue!40] (voice) at (4,-5) {Voice Processing\\Service};
    
    % External Services
    \node[cloud] (gemini) at (4,-7) {Gemini API};
    
    % Database
    \node[cylinder, draw, shape aspect=0.3, fill=SayNoteBlue!20] (db) at (0,-7) {Database};
    ...
    \end{tikzpicture}
    \caption{SayNote System Architecture}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        block/.style={rectangle, draw, rounded corners, minimum width=2.5cm, minimum height=1cm, align=center, fill=SayNoteLightGray},
        process/.style={rectangle, draw, minimum width=2.5cm, minimum height=1cm, align=center, fill=SayNoteLightGray},
        decision/.style={diamond, draw, minimum width=2.5cm, minimum height=1.5cm, align=center, fill=SayNoteLightGray},
        arrow/.style={->, >=stealth, thick, color=SayNoteBlue},
        every node/.style={font=\small}
    ]
    
    % Flow steps
    \node[block, fill=SayNoteBlue!10] (start) at (0,0) {User speaks\\command};
    \node[process, fill=SayNoteBlue!20] (capture) at (0,-2) {Voice capture};
    \node[process, fill=SayNoteBlue!30] (transcribe) at (0,-4) {Speech-to-text\\transcription};
    \node[process, fill=SayNoteBlue!40] (intent) at (0,-6) {Intent parsing\\(Gemini API)};
    \node[decision, fill=SayNoteBlue!30] (valid) at (0,-8.5) {Valid\\command?};
    \node[process, fill=SayNoteBlue!20] (execute) at (0,-11) {Execute command\\in editor};
    \node[block, fill=SayNoteBlue!10] (feedback) at (0,-13) {User feedback};
    
    % Error path
    \node[process, fill=SayNoteGray!40] (error) at (4,-11) {Error handling\\and feedback};
    ...
    \node[draw, rounded corners, text width=4.5cm, fill=SayNoteDarkGray, text=white, font=\ttfamily\footnotesize] (json) at (-5,-6) {
        \{\\
        \ \ "intent": "FORMAT\_TEXT",\\
        \ \ "style": "bold",\\
        \ \ "text": "important",\\
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
        entity/.style={rectangle, draw, rounded corners, minimum width=3cm, minimum height=1.8cm, align=center, fill=SayNoteLightGray},
        relationship/.style={diamond, draw, minimum width=2cm, minimum height=1cm, align=center, fill=SayNoteBlue!20},
        line/.style={-, thick},
    ...
    % Entities
    \node[entity, fill=SayNoteBlue!20] (user) at (0,0) {
        \textbf{User}\\
        \rule{2.5cm}{0.5pt}\\
        id\\
        email\\
    ...
    \node[entity, fill=SayNoteBlue!20] (note) at (6,0) {
        \textbf{Note}\\
        \rule{2.5cm}{0.5pt}\\
        id\\
        title\\
    ...
    \node[entity, fill=SayNoteBlue!20] (block) at (12,0) {
        \textbf{Block}\\
        \rule{2.5cm}{0.5pt}\\
        id\\
        type\\
    ...
    \node[entity, fill=SayNoteBlue!20] (folder) at (0,-5) {
        \textbf{Folder}\\
        \rule{2.5cm}{0.5pt}\\
        id\\
        name\\
    ...
    \node[entity, fill=SayNoteBlue!20] (tag) at (6,-5) {
        \textbf{Tag}\\
        \rule{2.5cm}{0.5pt}\\
        id\\
        name\\
    ...
    \node[entity, fill=SayNoteBlue!20] (template) at (12,-5) {
        \textbf{Template}\\
        \rule{2.5cm}{0.5pt}\\
        id\\
        name\\
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
        component/.style={rectangle, draw, rounded corners, minimum width=2.5cm, minimum height=1cm, align=center, fill=SayNoteLightGray},
        screen/.style={rectangle, draw, rounded corners, minimum width=2.5cm, minimum height=1cm, align=center, fill=SayNoteBlue!20},
        arrow/.style={->, >=stealth, thick, color=SayNoteBlue},
        every node/.style={font=\small}
    ]
    ...
    \node[anchor=north west, font=\bfseries] at (-1,1) {SayNote App};
    
    % Navigation
    \node[component, fill=SayNoteBlue!10, minimum width=12cm] (nav) at (6.5,0) {Bottom Tab Navigator};
